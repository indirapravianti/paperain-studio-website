...
import { handleShippingCoverage } from '../lib/api/shipping-coverage-handler.js';
...
export default async function handler(req, res) {
  ...
  try {
    const result = await handleShippingCoverage(parseRequestBody(req));
    return sendJson(res, result.body, result.status);
  } catch (err) {
    console.error('api/shipping-coverage error:', err);
