...
import { handleShippingTracking } from '../lib/api/shipping-tracking-handler.js';
...
export default async function handler(req, res) {
  ...
  try {
    const result = await handleShippingTracking(parseRequestBody(req));
    return sendJson(res, result.body, result.status);
  } catch (err) {
    console.error('api/shipping-tracking error:', err);
