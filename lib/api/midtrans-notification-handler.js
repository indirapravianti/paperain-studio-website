...
import { confirmStockForOrder, restoreStockForOrder } from './place-order-handler.js';
import { createKiriminajaShipmentForOrder } from './kiriminaja-shipment-handler.js';
import { kiriminajaShipmentConfigured } from './kiriminaja.js';
...
export async function handleMidtransNotification(body) {
  ...
  if (!skipUpdate) {
    ...
    if (newStatus === 'confirmed') {
      await confirmStockForOrder(supabase, order.id);
    }
  ...
  if (newStatus === 'confirmed') {
    ...
    if (kiriminajaShipmentConfigured()) {
      const shipmentResult = await createKiriminajaShipmentForOrder(supabase, order.id);
      if (!shipmentResult.ok) {
        console.error('kiriminaja shipment creation failed:', order.id, shipmentResult.error);
      }
    }
  }
