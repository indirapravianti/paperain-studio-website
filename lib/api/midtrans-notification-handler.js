...
export async function handleMidtransNotification(body) {
  ...
  if (newStatus === 'confirmed') {
    const shipmentResult = await createShipmentForOrder(supabase, order.id);
    if (shipmentResult.ok && shipmentResult.awb) {
      console.log('KiriminAja shipment created:', order.id, shipmentResult.awb);
    } else if (!shipmentResult.skipped) {
      console.error('KiriminAja shipment creation failed:', order.id, shipmentResult);
    }
