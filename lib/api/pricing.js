...
/** Approximates a single box's dimensions as the max per-axis size across line items (not real packing). */
export async function resolvePackageDimensions(supabase, lines) {
  const productIds = [...new Set(lines.map((line) => line.product_id))];
  if (!productIds.length) {
    return { width: DEFAULT_WIDTH_CM, height: DEFAULT_HEIGHT_CM, length: DEFAULT_LENGTH_CM };
  }

  const { data: products } = await supabase
    .from('products')
    .select('id, width_cm, height_cm, length_cm')
    .in('id', productIds);

  const dimsMap = new Map((products || []).map((p) => [p.id, p]));

  return lines.reduce(
    (max, line) => {
      const dims = dimsMap.get(line.product_id);
      return {
        width: Math.max(max.width, dims?.width_cm ?? DEFAULT_WIDTH_CM),
        height: Math.max(max.height, dims?.height_cm ?? DEFAULT_HEIGHT_CM),
        length: Math.max(max.length, dims?.length_cm ?? DEFAULT_LENGTH_CM),
      };
    },
    { width: DEFAULT_WIDTH_CM, height: DEFAULT_HEIGHT_CM, length: DEFAULT_LENGTH_CM },
  );
}
