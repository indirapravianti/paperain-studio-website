...
import { toMidtransCountryCode } from './country-codes.js';
...
export async function handleCreatePayment(body) {
  ...
  const midtransPayload = {
    ...
    customer_details: {
      ...
      shipping_address: {
        ...
        country_code: toMidtransCountryCode(order.shipping_country),
      },
    },
  };
  ...
  if (!snapRes.ok || !snapData.token) {
    ...
    const isClientError = Array.isArray(snapData.error_messages);
    return {
      status: isClientError ? 400 : 502,
      body: { error: message, detail: snapData },
    };
  }
